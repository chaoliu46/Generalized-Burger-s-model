function BackPeakJ2log=BackPeakJ2function(PeriodTemperatureWaterGrainsize,X,a)
%fit Burgers model with only background with a=2 and only peak with a=3 and together with a=1
%X is the fitting parameter 
...
J2=zeros(numel(Period),1);
BackPeakJ2log=zeros(numel(Period),1);
...
% Peak(i)=peak0+rWaterPt.*0.97332.^Water(i);
...
% deltaPeak1(i)=Tol(Temperature(i),Water(i),Grainsize(i),[deltaPeak,0,-rWaterP,Es]);
deltaPeak1(i)=deltaPeak.*(Water(i)/200)^rWaterP;
 ...
 if a==1
    ...
    J2(i)=Ju(Temperature(i),JuT,Ju0).*2*pi*Period(i)*(InterBackground(i)+InterPeak(i))+Ju(Temperature(i),JuT,Ju0).*Period(i)./(2.*pi.*Tolita(i));
%      J2(i)=2*pi*Period(i)*(InterBackground(i)+InterPeak(i))+Ju(Temperature(i),JuT,Ju0).*Period(i)./(2.*pi.*Tolita(i));
 end
 if a==2
     InterPeak(i)=0;
     InterBackground(i)=delta(i).*integral(@(tol) FunBackground(tol,alpha,Period(i)),Tollow(i),Tolhigh(i));
     J2(i)=Ju(Temperature(i),JuT,Ju0).*2*pi*Period(i)*(InterBackground(i)+InterPeak(i))+Ju(Temperature(i),JuT,Ju0).*Period(i)./(2.*pi.*Tolita(i));
%      J2(i)=2*pi*Period(i)*(InterBackground(i)+InterPeak(i))+Ju(Temperature(i),JuT,Ju0).*Period(i)./(2.*pi.*Tolita(i));
%      J2(i)=2*pi*Period(i)*(InterBackground(i)+InterPeak(i));
 end
 if a==0
      J2(i)=Ju(Temperature(i),JuT,Ju0).*Period(i)./(2.*pi.*Tolita(i));
%       InterBackground(i)=delta(i).*integral(@(tol) FunBackground(tol,alpha,Period(i)),Tollow(i),Tolhigh(i));
 ...
 if a==3
    InterPeak(i)=deltaPeak1(i).*integral(@(tol)FunPeak(tol,Period(i),theta,Peak(i)),0,inf);
     InterBackground(i)=0;
     J2(i)=Ju(Temperature(i),JuT,Ju0).*2*pi*Period(i)*(InterBackground(i)+InterPeak(i));
 end
 
%  --------------------------------------------------------------------------------------------------------

BackPeakJ2log(i)=log10(J2(i));
